...
const S={
  save:async(k,v)=>{try{await window.storage.set(k,JSON.stringify(v));}catch(e){console.warn(e);}},
...
export default function App(){
  ...
  useEffect(()=>{
    Promise.all([S.load(KEYS.p),S.load(KEYS.s),S.load(KEYS.e),S.load(KEYS.a),S.load(KEYS.cfg),S.load(KEYS.sv),S.load(CRM_KEYS.crm)]).then(([pd,sd,ed,ad,cfg,sv,crmd])=>{
      if(pd&&Array.isArray(pd))setProspects(pd);
      if(sd&&Array.isArray(sd)&&sd.length>0){
        // re-seed step 1 delayDays for any built-in sequence that has it wrong (>0)
        const fixed=sd.map(seq=>{
          const bi=BUILTIN.find(b=>b.name===seq.name);
          if(!bi||!seq.builtIn)return seq;
          // re-seed if step 1 delayDays wrong (>0) OR any same-day step has delayDays>0 when delta is 0
          const fresh=bsToSteps(bi.steps);
          const needsUpdate=seq.steps.some((s,i)=>s.delayDays!==fresh[i]?.delayDays);
          if(!needsUpdate)return seq;
          return{...seq,steps:fresh};
        });
        setSequences(fixed);S.save(KEYS.s,fixed);
      }
      else{const s=BUILTIN.map(seqToStore);setSequences(s);S.save(KEYS.s,s);}
      setEnrollments([]);S.save(KEYS.e,[]);
      setActivities({});S.save(KEYS.a,{});
      if(cfg)setSettings(cfg);
      if(sv&&Array.isArray(sv))setSavedViews(sv);
      if(crmd&&Array.isArray(crmd))setCrmRecords(crmd);
      // Load CRM from Supabase
      sbFetch("crm_records?order=created_at.asc").then((rows:any[])=>setCrmRecords(rows.map(fromRow))).catch(()=>{});
      setLoaded(true);
    });
  },[]);

  const flash=(m,t="ok")=>{setToast({m,t});setTimeout(()=>setToast(null),3000);};
  const upd=useCallback((id,patch)=>{setProspects(prev=>{const next=prev.map(p=>p.id===id?{...p,...patch}:p);setSaving(true);S.save(KEYS.p,next).finally(()=>setSaving(false));return next;});},[]);
  const setP=useCallback(fn=>{setProspects(prev=>{const next=typeof fn==="function"?fn(prev):fn;setSaving(true);S.save(KEYS.p,next).finally(()=>setSaving(false));return next;});},[]);
  const saveEnr=useCallback(async arr=>{setEnrollments(arr);await S.save(KEYS.e,arr);},[]);
  const logAct=useCallback((pid,obj)=>{const entry={id:uid(),prospectId:pid,ts:nowISO(),userName,meta:{},...obj};setActivities(prev=>{const next={...prev,[pid]:[entry,...(prev[pid]||[]).slice(0,99)]};S.save(KEYS.a,next);return next;});},[userName]);
  ...
  const commitPublish=useCallback(seq=>{
    setSequences(prev=>{const e=prev.find(s=>s.id===seq.id);const next=e?prev.map(s=>s.id===seq.id?seq:s):[...prev,seq];S.save(KEYS.s,next);return next;});
  ...
  const deleteSequence=useCallback(seqId=>{
    ...
    setSequences(prev=>{const next=prev.filter(s=>s.id!==seqId);S.save(KEYS.s,next);return next;});flash("Deleted","warn");
  },[enrollments]);
  ...
  const deleteView=useCallback(id=>{const next=savedViews.filter(v=>v.id!==id);setSavedViews(next);S.save(KEYS.sv,next);if(activeView===id)setActiveView(null);},[savedViews,activeView]);
  ...
  return(
    <div style={{fontFamily:"'DM Mono','Courier New',monospace",background:"#05080f",minHeight:"100vh",color:"#dde4ef",fontSize:12}}>
      ...
      {settingsOpen&&<SettingsModal settings={settings} onClose={()=>setSettingsOpen(false)} onSave={wh=>{setSettings(wh);S.save(KEYS.cfg,wh);setSettingsOpen(false);flash("Saved!");}}/>}
